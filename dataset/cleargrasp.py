...
class ClearGraspRealWorld(Dataset):
    ...
    def __init__(self, data_dir, split = 'test', **kwargs):
        ...
        self.split = split
        if split == 'train':
            raise AttributeError('Cleargrasp training is currently not supported.')
        self.image_paths = []
        self.depth_paths = []
        self.mask_paths = []
        self.depth_gt_paths = []
        for data_type in ['real-test', 'real-val']:
            for camera_type in ['d415', 'd435']:
                cur_path = os.path.join(data_dir, data_type, camera_type)
    ...
    def __getitem__(self, id):
        ...
        depth = exr_loader(self.depth_paths[id], ndim = 1)
        depth_gt = exr_loader(self.depth_gt_paths[id], ndim = 1)
